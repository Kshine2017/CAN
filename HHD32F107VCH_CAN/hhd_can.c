...
//#define OSTimeDly(a) (void)a;


extern uint32_t SetTick(uint32_t ms);
extern uint32_t GetTick(void);
...
int can_write(HHD32F1_CAN_TypeDef *can, enum FRMAT_TYPE type, union UN_CAN_IDENTIFY *Identifier,  uint8_t *data, int len)
{
	...
	//bit5 TS 传输状态                  1 CAN 控制器处于发送消息进程中                           0 无消息在发送中
	while(CAN_GetStatus(can) & TranSt){
			if(!GetTick())
					return -1;
	}
	
	//bit2 TBS 传输 buffer状态          1 发送 buffer 已被释放，CPU 可以写入新消息               0 发送 buffer 被锁定，buffer 中消息正准备被发送
	while(!(CAN_GetStatus(can) & TranBufSt)){
			if(!GetTick())
					return -1;
	}
	...
	can->DATAINFO.TxDATAINFO.TxDATA[0].all = data[0];  //data 1
	can->DATAINFO.TxDATAINFO.TxDATA[1].all = data[1];  //data 2
	can->DATAINFO.TxDATAINFO.TxDATA[2].all = data[2];  //data 3
	can->DATAINFO.TxDATAINFO.TxDATA[3].all = data[3];  //data 4
	can->DATAINFO.TxDATAINFO.TxDATA[4].all = data[4];  //data 5
	can->DATAINFO.TxDATAINFO.TxDATA[5].all = data[5];  //data 5
	can->DATAINFO.TxDATAINFO.TxDATA[6].all = data[6];  //data 7
	can->DATAINFO.TxDATAINFO.TxDATA[7].all = data[7];  //data 8
	...
	//bit3 TCS 传输完成状态             1 最后一笔数据已被发送完成                               0 最后一笔数据未被发送完成
	while(!(CAN_GetStatus(can) & TranComplete)){
		if(!GetTick())
				return -1;
	}
	
	//清空发送标记位
	CAN_ClrCMD(can,TransReq);
...
int Ext_can_write(HHD32F1_CAN_TypeDef *can, union UN_CAN_IDENTIFY *Identifier,  uint8_t *data, int len)
{
	...
	SetTick(5);
	while((CAN_GetStatus(can) & RecSt) == RecSt ){if(!GetTick())return -1;}
	while((CAN_GetStatus(can) & TranComplete) != TranComplete ){if(!GetTick())return -1;}
	while((CAN_GetStatus(can) & TranBufSt) != TranBufSt ){if(!GetTick())return -1;}
...
//	can->DATAINFO.TxDATAINFO.TxFRAME.bit.RTR = 0;//不是远程帧
	...
	for(i = 0; i < len; i++)
	{
		...
		CAN_SetCMD(can, TransReq);                             		// 启动发送
		while((CAN_GetStatus(can) & TranComplete) != TranComplete ){if(!GetTick())return -1;}	//等待发送完成
		while((CAN_GetStatus(can) & TranBufSt) != TranBufSt ){if(!GetTick())return -1;}	    // 等待发送缓冲被释放
	return 0;
}
/***********************************************************************************************
*
